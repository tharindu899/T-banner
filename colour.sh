...
if [ -f "$HOME/.termux/font.ttf" ]; then
    ...
    cp -r "$HOME/T-banner/.object/font.ttf" "$HOME/.termux"
    sleep 3
...
copy_file() {
    ...
    if [ -e "$HOME/.termux/termux.properties" ]; then
        ...
        cp -r "$HOME/T-banner/.object/termux.properties" "$HOME/.termux/termux.properties" > /dev/null 2>> "$ERROR_LOG" &
    ...
    if [ ! -e "$PREFIX/etc/motd1" ]; then
        ...
        rm -rf zshrc
        cp -r "$HOME/T-banner/.object/zshrc" "$PREFIX/etc/zshrc" > /dev/null 2>> "$ERROR_LOG" &
        spin $! "zshrc"
    fi
  ...
  # Download .zshrc and .p10k.zsh
    ...
    if [ ! -e "$HOME/.zshrc" ]; then
        cp -r "$HOME/T-banner/.object/.zshrc" "$HOME/.zshrc" > /dev/null 2>> "$ERROR_LOG" &
        spin $! "zshrc"
    fi
    
    check_exist "$HOME/.p10k.zsh" ".p10k.zsh "
    if [ ! -e "$HOME/.p10k.zsh" ]; then
        cp -r "$HOME/T-banner/.object/.p10k.zsh" "$HOME/.p10k.zsh" > /dev/null 2>> "$ERROR_LOG" &
        spin $! "p10k.zsh"
    fi
    
    if [ -e "$HOME/.banner.sh" ]; then
        rm "$HOME/.banner.sh"
        cp -r "$HOME/T-banner/.object/banner.sh" "$HOME/.banner.sh" > /dev/null 2>> "$ERROR_LOG" &
        spin $! "termux.properties"
    else
        cp -r "$HOME/T-banner/.object/banner.sh" "$HOME/.banner.sh" > /dev/null 2>> "$ERROR_LOG" &
        spin $! "termux.properties"
